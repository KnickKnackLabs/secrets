...
@test "keychain_set stores base64-encoded value in keychain" {
  ...
  local service="${SECRETS_SERVICE_PREFIX}github-pat"
  local stored
  stored=$(cat "$MOCK_KEYCHAIN/test-agent/$service")
  local decoded
...
@test "keychain_get isolates agents — different agents have different values" {
